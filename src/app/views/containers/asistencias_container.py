...
# ✅ Helper global de ordenamientos
...
class AsistenciasContainer(ft.Container):
    ...
    - Ley: los registros INCOMPLETOS siempre van arriba.
    - Sworting por encabezados por PANEL: Nómina, Nombre, Fecha, Hora Entrada, Hora Salida, Estado.
    - Caja "Ordenar por ID nómina": al presionar Enter sube todas las filas de ese ID;
      si limpias el campo, vuelve automáticamente al orden normal.
    - Caja "Buscar por Nombre": al presionar Enter prioriza filas cuyo nombre coincida
      (contiene, sin acentos, no sensible a mayúsculas). Si no hay coincidencias,
      muestra un aviso "esta busqueda no esta disponible".
    """
    ...
    def __init__(self):
        ...
        # Índices por campo para localizar celdas (p.ej., "estado")
        ...
        # ---- Toolbar: Ordenar por ID nómina / Buscar por Nombre ----
        self.sort_id_input = ft.TextField(
            label="Ordenar por ID nómina",
            hint_text="Escribe un ID y presiona Enter",
            width=220,
            keyboard_type=ft.KeyboardType.NUMBER,
            on_submit=self._aplicar_sort_id,          # Enter aplica prioridad por ID
            on_change=self._id_on_change_auto_reset,  # vaciar → vuelve al orden normal
        )
        ...
        self.sort_name_input = ft.TextField(
            ...
            width=320,
            on_submit=self._aplicar_sort_nombre,              # Enter aplica búsqueda por nombre
            on_change=self._nombre_on_change_auto_reset,      # vaciar → vuelve al orden normal
        )
        ...
        self._import_export_row = ft.Row([self.import_button, self.export_button],
    ...
    def _nombre_coincide(self, registro: dict) -> bool:
        if not self.sort_name_filter:
            return False
        needle = Sworting.normalize_text(self.sort_name_filter or "")
        haystack = Sworting.normalize_text(registro.get("nombre_completo", ""))
    ...
    def _ordenar_lista(self, datos: list[dict], grupo: str) -> list[dict]:
        ...
        if key == "numero_nomina":
            ...
            ordered = sorted(ordered, key=lambda r: Sworting.normalize_text(r.get("nombre_completo", "")), reverse=not asc)
        elif key == "fecha":
        ...
        if self.sort_name_filter:
            ...
            ordered = sorted(ordered, key=lambda r: 0 if needle in Sworting.normalize_text(r.get("nombre_completo", "")) else 1)
    ...
    def _actualizar_tabla(self):
        ...
        # ✅ AGRUPAR (el orden intra-grupo se aplicará más abajo por PANEL)
        self.datos_por_grupo = self._agrupar_por_grupo_importacion(datos)

        # Reordenar grupos: si hay filtro por ID o por Nombre → grupos con match arriba; luego por fecha desc
        ...
        # 👉 Manejo robusto de expansión de grupos
        if not self.grupos_expandido:
            # Primera carga → expandir los que tienen incompletos
            ...
            # Mantener estado previo, añadir grupos nuevos como colapsados
        ...
        for grupo, registros in grupos_ordenados:
            ...
            # Orden dentro del grupo (usa _ordenar_lista con estado_rank)
            registros_ordenados = self._ordenar_lista(list(registros), grupo)

            filas = []
            for reg in registros_ordenados:
                # Solo sobrescribimos si está en error o incompleto
                if reg.get("__error_horas", False):
                    reg["estado"] = "ERROR"
                elif Sworting.is_asistencia_incomplete(reg):
                    reg["estado"] = "INCOMPLETO"
                # else → respetar lo que ya tenga en DB
        ...
        # Reemplaza solo la lista de paneles
        self._root_column.controls = [self._title_label, self._import_export_row, self._toolbar_row, epl]
    ...
    def _estilizar_fila(self, fila: ft.DataRow, editable: bool, incompleto: bool):
        ...
        for idx, cell in enumerate(fila.cells):
            if idx >= len(self.columnas_definidas):
                # Evita IndexError si hay más celdas de las esperadas
                continue
    ...
    def _agregar_fila_en_grupo(self, grupo_importacion):
        # Descanso por defecto = "MD" (medio tiempo)
        self.editando[("nuevo", grupo_importacion)] = {
            "numero_nomina": "",
            "fecha": "",
            "hora_entrada": "",
            "hora_salida": "",
            "descanso": "MD",          # 👈 default medio descanso
            "tiempo_trabajo": "0.00",
            "estado": "PENDIENTE",
        }
    ...
    def _toggle_expansion(self, grupo):
        # 🔑 Alternar solo ese grupo, sin cerrar los demás
    ...
    def _guardar_edicion(self, numero_nomina, fecha):
        ...
        # ✅ Solo forzamos a INCOMPLETO si aplica, sino dejamos el estado original de DB
    ...
    def _confirmar_eliminar_grupo(self, grupo):
        try:
            ...
            # Notificar calendario por cada fecha que ahora podría quedar libre
    ...
    def _eliminar_asistencia(self, numero, fecha):
        try:
            ...
            if resultado["status"] == "success":
                ...
                # Si la fecha quedó sin registros → reabrir en calendario
    ...
    def _notificar_fecha_libre_si_corresponde(self, fecha_str: str):
        ...
        try:
            res = self.asistencia_model.get_all()
            if res.get("status") == "success":
                quedan = any(str(r.get("fecha")) == fecha_str for r in res.get("data", []))
                if not quedan and self.page:
                    # Canal genérico de pubsub; tu módulo de calendario puede escuchar este evento
