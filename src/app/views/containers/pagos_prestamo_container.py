...
class PagosPrestamoContainer(ft.Container):
    def __init__(self):
        ...
        # ✅ Invokers configurados correctamente
        ...
        self.boton_importar = self._boton_estilizado("Importar", ft.icons.FILE_UPLOAD, lambda _: self.importador.open())
        self.boton_exportar = self._boton_estilizado("Exportar", ft.icons.FILE_DOWNLOAD, lambda _: self.exportador.open_save())
    ...
    def _cargar_pagos(self, id_prestamo: int):
        ...
        for p in resultado["data"]:
            ...
            self.tabla_pagos.rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(p[self.E.ID_PAGO_PRESTAMO.value])),
                ...
                ft.DataCell(ft.IconButton(
                    ...
                    on_click=lambda e, pid=p[self.E.ID_PAGO_PRESTAMO.value]: self._eliminar_pago(pid)
                ))
            ]))
    ...
    def _procesar_importacion(self, ruta_archivo: str):
        """Importa pagos desde Excel y actualiza la vista."""
        try:
            df = pd.read_excel(ruta_archivo)
            columnas_requeridas = ["Monto Pagado", "Fecha Generación", "Interés %", "Fecha Real", "Observaciones"]
            ...
            for _, row in df.iterrows():
                ...
                existente = self.pago_model.db.get_data(
                    f"""SELECT COUNT(*) AS c FROM {self.E.TABLE_PAGOS_PRESTAMOS.value}
            ...
            ModalAlert.mostrar_info("Importación finalizada", mensaje)
    ...
    def _exportar_pagos(self, path: str = None):
        """Exporta todos los pagos del préstamo a Excel (.xlsx)."""
        try:
            ...
            columnas = [
                (self.E.ID_PAGO_PRESTAMO.value, "ID Pago"),
