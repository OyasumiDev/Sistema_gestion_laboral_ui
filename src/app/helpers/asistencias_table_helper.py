...
class AsistenciasTableHelper:
    def __init__(self, on_update_page, on_guardar_fila, on_cancelar_fila, on_editar, on_guardar_edicion, on_cancelar_edicion):
        self.on_update_page = on_update_page
    ...
    def crear_fila_nueva(self, registro: dict, grupo_importacion: str, on_update_valor):
        return ft.DataRow(cells=[
            ft.DataCell(self._crear_textfield(registro.get("numero_nomina", ""), 100, lambda e: on_update_valor(grupo_importacion, "numero_nomina", e.control.value, "nuevo"))),
            ft.DataCell(ft.Text("-")),
            ft.DataCell(self._crear_textfield(registro.get("fecha", ""), 100, lambda e: on_update_valor(grupo_importacion, "fecha", e.control.value, "nuevo"))),
            ft.DataCell(self._crear_textfield(registro.get("hora_entrada", ""), 100, lambda e: on_update_valor(grupo_importacion, "hora_entrada", e.control.value, "nuevo"))),
            ft.DataCell(self._crear_textfield(registro.get("hora_salida", ""), 100, lambda e: on_update_valor(grupo_importacion, "hora_salida", e.control.value, "nuevo"))),
            ft.DataCell(self._crear_botones_descanso(on_update_valor, grupo_importacion=grupo_importacion, tipo_edicion="nuevo")),
            ft.DataCell(self._crear_textfield(registro.get("tiempo_trabajo", "0.00"), 80, read_only=True)),
    ...
    def crear_fila_edicion(self, registro: dict, on_update_valor):
        ...
        return ft.DataRow(cells=[
            ...
            ft.DataCell(self._crear_textfield(registro.get("hora_entrada", ""), 100, lambda e: on_update_valor(numero_nomina, fecha, "hora_entrada", e.control.value, "edicion"))),
            ft.DataCell(self._crear_textfield(registro.get("hora_salida", ""), 100, lambda e: on_update_valor(numero_nomina, fecha, "hora_salida", e.control.value, "edicion"))),
    ...
    def _crear_botones_descanso(self, on_update_valor, **kwargs):
        ...
        if tipo_edicion == "nuevo":
            ...
            numero_nomina = kwargs["numero_nomina"]
            fecha = kwargs["fecha"]
            for tipo in ["SN", "MD", "CMP"]:
                botones.append(
                    ft.FilledButton(
